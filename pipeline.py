...
def pipeline(k=10, api_key=None, base_dir="example_data", font="Times New Roman"):
    ...
    for i in range(k):
        if text_colors[i] == background_colors[i]:
            new_possible_text_colors = [
                color for color in possible_text_colors if color != text_colors[i]
            ]
    ...
    for i, (
        ...
        if response:
            filename = (
                f"exercise_{lang}_{num}_{flag}_{text_color}_{background_color}_"
            ...
            convert_pdf_to_png(
                pdf_path=generated_dir + f"/{i}/{filename}.pdf",
...
if __name__ == "__main__":
    ...
    pipeline(k=10, api_key=api_key, base_dir="example01", font="Times New Roman")
